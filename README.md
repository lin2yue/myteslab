...
使用方法：
1. 用 VS Code Live Server 打开 `dev-studio/tweak.html`
2. 选择车型和贴图
3. 调整参数并导出配置到 `viewer-config.json`
...
### Web 版
- `apps/web/src/config/viewer-config.json` - 3D 查看器配置（UV、缩放、旋转）
- `apps/web/.env.local` - 环境变量（Supabase、CDN）

### 小程序版
- `apps/miniprogram/.env` - 环境变量
...
### Q: 为什么没有 packages/shared？
A: 两个应用当前架构差异大（Next.js vs uni-app），强行共享代码会增加复杂度。采用"按需提取"策略，等真正出现重复代码再重构。
