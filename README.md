...
**核心原则**：所有动态生成的贴图（AI 或 DIY）必须按标准方向输出。3D 查看器默认信任资产方向，不再进行二次旋转补偿。

| 车型 | 官方规格 | 旋转逻辑 (从 AI 构图转回) | 实现方式 |
|------|----------|-------------------------|----------|
| **Model 3 / Y** | 1024x1024, **车头朝上** | 顺时针旋转 **180°** | `?x-oss-process=image/rotate,180` |
| **Cybertruck** | 1024x768, **车头朝左** | 顺时针旋转 **90°** | `?x-oss-process=image/rotate,90` |

> [!TIP]
> **AI 生图优化**：系统已将 CPU 密集型的旋转计算迁移至 OSS 云端。API 仅存储带处理参数的 URL。
> **参数合并**：UI 组件加载缩略图（WebP）时会自动将业务参数与预设旋转参数合并。

> [!CAUTION]
> **严禁**在 `ModelViewer.tsx` 或 `viewer-config.json` 中针对动态资产添加物理旋转偏移。
