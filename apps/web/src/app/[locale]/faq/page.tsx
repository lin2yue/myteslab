...
export const metadata = {
    title: 'FAQ - Tesla Wrap Designs | MyTesLab',
...
export default async function FAQPage({
    ...
    const faqs = [
        ...
        {
            question: locale === 'en' ? 'What is MyTesLab Tesla Wrap Gallery?' : '什么是 MyTesLab 特斯拉贴膜库？',
            answer: locale === 'en'
                ? 'MyTesLab is a free online platform where you can browse, create, and download custom wrap designs for Tesla vehicles including Model 3, Model Y, Cybertruck, Model S, and Model X. All designs feature real-time 3D preview and are free to download. Perfect for Tesla Toybox Paint Shop.'
                : 'MyTesLab 是一个免费在线平台，您可以浏览、创建和下载特斯拉车辆的定制贴膜设计，包括 Model 3、Model Y、Cybertruck、Model S 和 Model X。所有设计都具有实时 3D 预览功能，并且免费下载。完美适配 Tesla Toybox Paint Shop。'
        },
        {
        ...
        },
        {
            ...
            answer: locale === 'en'
                ? '1. Download your favorite wrap design from MyTesLab. 2. Format a USB drive as exFAT or FAT32. 3. Create a folder named "Wraps" (exact spelling) at the root of the USB drive. 4. Copy the downloaded PNG file to the Wraps folder. 5. Insert the USB drive into your Tesla. 6. Go to Toybox → Paint Shop → Wraps on your car\'s touchscreen. 7. Select your custom wrap to apply it.'
                : '1. 从 MyTesLab 下载您喜欢的贴膜设计。2. 将 U 盘格式化为 exFAT 或 FAT32。3. 在 U 盘根目录创建名为"Wraps"的文件夹（拼写必须准确）。4. 将下载的 PNG 文件复制到 Wraps 文件夹。5. 将 U 盘插入特斯拉。6. 在车机触摸屏上进入 Toybox → Paint Shop → Wraps。7. 选择您的定制贴膜应用。'
        },
        {
            question: locale === 'en' ? 'What file format do I need for Tesla Toybox wraps?' : 'Tesla Toybox 贴膜需要什么文件格式？',
            answer: locale === 'en'
                ? 'Tesla Toybox requires PNG files with specific requirements: File size must be under 1MB, resolution between 512x512 and 1024x1024 pixels, and file name should use only alphanumeric characters, spaces, underscores, or dashes (max 30 characters). All MyTesLab downloads are pre-optimized to meet these requirements.'
                : 'Tesla Toybox 需要符合特定要求的 PNG 文件：文件大小必须小于 1MB，分辨率在 512x512 到 1024x1024 像素之间，文件名只能使用字母数字、空格、下划线或破折号（最多 30 个字符）。所有 MyTesLab 下载都已预先优化以满足这些要求。'
        },
        {
