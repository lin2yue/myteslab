    ...
    <priority>${priority}</priority>`
            ...
            if (page === '' && homePageWraps.length > 0) {
                for (const wrap of homePageWraps) {
                    ...
                    const imgTitle = `${wrap.name} - 特斯拉 ${modelName} 贴膜设计`
                    const imgCaption = `适用于特斯拉 ${modelName} 的 ${wrap.name} 车身贴膜设计，免费下载。`
    ...
    // 2. Model Pages
        ...
        try {
            ...
            for (const model of models) {
                const url = `${baseUrl}/models/${model.slug}`
                xml += `  <url>
    <loc>${url}</loc>
    <lastmod>${new Date().toISOString()}</lastmod>
    <changefreq>weekly</changefreq>
    ...
    // 3. Wrap Pages (Pagination)
        ...
        try {
            ...
            for (const wrap of pagedWraps) {
                const url = `${baseUrl}/wraps/${wrap.slug}`
                ...
                // 动态增强图片 SEO 描述
                const imgTitle = `${wrap.name} - 特斯拉 ${modelName} 贴膜预览`
                const imgCaption = `适用于特斯拉 ${modelName} 的高端 ${wrap.name} 车身贴膜设计图。专业级定制纹理模板，支持 3D 可视化预览，提供免费高清下载。`
