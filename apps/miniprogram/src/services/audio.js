...
export const audioService = {
    async fetchAudios(sortBy = 'created_at', page = 1, pageSize = 20, searchQuery = '') {
        ...
        // Apply range last to ensure we paginate the filtered/sorted results
    ...
    async incrementStat(id, type) {
        ...
        if (error) console.error('Error incrementing stat:', error);
    },
    ...
    getDownloadUrl(url, cacheKey = '') {
        ...
        const setQueryParam = (items, key, value) => {
            const encodedKey = encodeURIComponent(key);
            const encodedValue = encodeURIComponent(String(value));
            for (let i = 0; i < items.length; i += 1) {
                if (items[i][0] === encodedKey || items[i][0] === key) {
                    items[i] = [encodedKey, encodedValue];
                    return;
                }
            }
        ...
        if (cacheKey) setQueryParam(items, 'v', cacheKey);

        const finalQuery = items.length > 0 ? `?${items.map(([k, v]) => `${k}=${v}`).join('&')}` : '';
        // Use CDN for download if possible, or OSS if required. Use CDN generally.
