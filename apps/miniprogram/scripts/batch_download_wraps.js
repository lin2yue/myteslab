...
async function downloadFilePuppeteer(page, url, dest) {
    try {
        // Evaluate fetch in the page context
        const dataBase64 = await page.evaluate(async (url) => {
        ...
        throw new Error(`Puppeteer download failed: ${e.message}`);
    }
}

function ensureDir(dir) { if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); }

async function processTarget(target, page) {
    ...
    for (const wrapName of WRAPS) {
        ...
        if (!fs.existsSync(localPath)) {
            try {
                ...
                await downloadFilePuppeteer(page, remoteUrl, localPath);
                console.log('✅');
            } catch (e) {
        ...
        try {
            ...
            if (!wrapData) {
                ...
                await supabase.from('wraps').update({ wrap_image_url: publicUrl }).eq('id', wrapData.id);
            }
...
async function main() {
    ...
    // Visit the site once to set cookies/headers
    console.log("Navigating to site...");
    await page.goto('https://teslawrapgallery.com', { waitUntil: 'networkidle0' });

    try {
        for (const target of TARGETS) {
            await processTarget(target, page);
        }
        console.log('\n🎉 All Wraps Processed.');
    } finally {
        await browser.close();
